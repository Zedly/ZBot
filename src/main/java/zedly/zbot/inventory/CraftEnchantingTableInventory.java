...
import zedly.zbot.network.packet.serverbound.Packet0AClickContainerButton;
...
public class CraftEnchantingTableInventory extends CraftExternalInventory implements EnchantingTableInventory {
    ...
    public void enchant(int slot) {
        if (slot >= 0 && slot < 3) {
            context.getUpThread().sendPacket(new Packet0AClickContainerButton(windowId, slot));
        }
    }
}
