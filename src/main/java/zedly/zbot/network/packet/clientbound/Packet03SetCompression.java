...
/**
* Enables compression.  If compression is enabled, all following packets are encoded in the <a href="#With_compression">compressed packet format</a>.  Negative or zero values will disable compression, meaning the packet format should remain in the <a href="#Without_compression">uncompressed packet format</a>.  However, this packet is entirely optional, and if not sent, compression will also not be enabled (the notchian server does not send the packet when compression is disabled).
*/
